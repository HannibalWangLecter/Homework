...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		// with bfs, the first-found solution is the best solution!
		...
		// (2) if openList == null, no solution
		while (this.openList.isEmpty() != true) {
			
			// (2-1) visit and compare the first node
	 ...
	 *           solutionPath记录了解路径。
	 *  搜索过程和结果会记录在DemoASearchDialog.txt中。
	...
	private void estimateValue(JigsawNode jNode) {
		...
		int conWrong = 0; // incorrect node numbers
		int i = 0; // for loop
		int state[] = jNode.getNodesState();
		int dim = JigsawNode.getDimension();

		// calculate incorrect node in cols and rows
		int rowLength = dim*dim;
		int colLength = rowLength - dim;
		// total wrong node numbers

		for (i = 0; i<rowLength;i++) {
			if (state[i]+1 != state[i+1]) {
				conWrong++;
			}
			if ((i < colLength) && (state[i] != state[i+dim]-dim)) {
				conWrong++;
			}
		}

		// distance between wrong and correct pos
		int distWrong = 0;
		for ( i = 1; i <= rowLength; i++) {
			if (state[i] != i && state[i] != 0) {
				// calcualte
				int deltaRow = (state[i] - 1) / dim - (i - 1) / dim;
				int deltaCol = (state[i] - 1) % dim - (i - 1) % dim;

				distWrong += Math.abs(deltaRow) + Math.abs(deltaCol);
			}
		}

		int est = 3 *conWrong  + 5 * distWrong;
		jNode.setEstimatedValue(est);
